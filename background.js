...
brows.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  ...
  switch (request.type) {
    case "request_count_hiden_posts":
      ...
      case "request_count_delted_posts":
        {
          brows.storage.local.get("countDeletedPost", function (items) {
            if (!brows.runtime.error) {
              console.log(items);
  
            }
            brows.runtime.sendMessage({ type: "response_count_deleted_posts", count: items.countDeletedPost || 0 });
          });
    ...
    case "inc_hiden_posts":
      ...
      case "inc_delete_posts":
      {
        brows.storage.local.get("countDeletedPost", function (items) {
          if (!brows.runtime.error) {
            console.log(items);
            var count = items.countDeletedPost || 0;
            brows.storage.local.set({ "countDeletedPost": count + 1 }, function () {
              if (brows.runtime.error) {
                console.log("Runtime error.");
              } else {
                console.log("countDeletedPost Changed, new ", count + 1);
              }
            });
    ...
    case "change_action_hidden_post_delete_post":
      {
        brows.storage.local.get("hidePost", function (items) {
          if (!brows.runtime.error) {
            console.log(items);
          ...
          for (let i = 0; i < old.length; i++) {
            if (i == request.id) {
              old[i].isDeletePost = request.isDeletePost;
            }
    ...
    case "add_hiden_posts":
      {
        ...
        brows.storage.local.get("hidePost", function (items) {
          ...
          posts.push({ content: request.text })
